...
export default function Header(){
    ...
    return(
        <header className="fixed top-0 backdrop-blur-xl z-50 w-screen">
            ...
            <motion.nav animate={isOpen ? "open":"closed"} initial="closed" variants={variants} transition={{x: { bounce: 0 }, opacity: { duration: 0.2 }}} className={"fixed top-0 right-0 w-screen h-screen md:hidden bg"}>
                <div className="container">
                    ...
                    <div className="flex flex-col gap-3 text-neutral-white justify-center items-center">
                        <Link href={"/"} onClick={() => setOpen(!isOpen)} className="item">Home<BsArrowRight/></Link>
                        <Link href={"/about"} onClick={() => setOpen(!isOpen)} className="item">About<BsArrowRight/></Link>
                        <Link href={"/project"} onClick={() => setOpen(!isOpen)} className="item">Project<BsArrowRight/></Link>
                        <Link href={"/"} onClick={() => setOpen(!isOpen)} className="item">Contact<BsArrowRight/></Link>
